...
La aplicación del método reduce a un subconjunto de  dimensiones (componentes principales) la información original. En  análisis multivariado  el concepto *Synthetic variable*  denota la escencia del PCA. 

Sugerencia: Jean Paul Bencecri sobre orígenes de este vínculo entre Multivariate Analysis  DA y PCA. 
...
![](\img\pca1.jpg)
Note en este caso la linea central en el primer panel. esta captura la dirección por la que las dos variables se relacionan, en cierto sentido este vector sintetiza la realción entre las variables pobalción y  gasto en educación para una muestra de 100 ciudades.

Si tomamos este vector y lo rotamos de tal manera que sea paralelo al eje horizontal ahora vemos un segunda fuente de variación (un segundo vector que capura otra dimensión de la  variación en los datos) correspondiente a la variación vertical (representada por la distancia entre cada observación y nuestro  primer vector). 
...
Note que el punto azul representa las coordenadas del valor promedio de las variables población y gasto en educación.



El método permite estimar la proporción de la variación que cada dimensión (componentes principales), de tal forma que podamos identificar las variables de mayor relevancia para el análisis y conocer las variables correlacionadas. Esa variación generalmente se representa mediante un **Scree plot** ( **x** axis componentes principales, **y** axis proporción de la varianza explicada).
...
El número de Eigenvectores a rentener es una cuestión empírica que depende del escenario de análisis, si bien un *Threshold* común es considerar aquellos pc que aportan $\geq 85\%$ de la varianza en los datos. de manera alternativa  conservar aquellos Eigenvectores con Eigenvalores  $\lambda\geq1$.
...
Estos permiten determinar la probabilidad de pertenencia a una categoría determinada dadas las característcas de cada observación.

El domino de aplicación es amplio. Una aproximación inicial al su uso es en la visualización, un proceso fundamental para comprender como se comportan nnuestros datos antes de aplicar modelos de mayor complejidad. (VSM Vector Support Machine, NN Neural Networks , RF Random Forest,etc.,).
...
Suponga que tiene la sigueinte  matriz con dimensiones mxn   m=5, n=3
...
Note que el producto resultante la matriz transformada cambió el tamaño,  no la dirección del vector. 
...
Donde **v** es un Eigenvector, $\lambda$ el Eigenvalor, A una matriz cuadrada. 
...
En general la imagen de un vector **x** bajo  multiplicación por una matriz **A** difiere tanto en dirección como en magnitud. Sin emabargo en el caso especial donde x es un Eigenvector de A; multiplicación por A deja la dirección sin cambio, Por ejemplo en   $\mathbb{R}^2$ o $\mathbb{R}^3$ multiplicación por A proyecta cada eigenvalor **x** de **A** (en caso de existir),  a lo largo de la misma linea de **x** que atraviesa el origen. Dependiendo del signo y magnitud del eigenvalor $\lambda$ correspondiente a **x**, la operacción $Ax=\lambda x$ comprime o expande **x** por un factor igual a $\lambda$, con una dirección inversa en el caso de que $\lambda>0$.

La siguente figura disponible en Anton H. ilustra este efecto.
...
El proceso de PCA se basa en estos fundamentos teroricos para la determinación de los eigenvectores y corresppodientes eigenvalores de la matriz de datos.

Entonces los eigenvectores capturan la "dirección" principal en las que ubican los datos en el espacio vectorial y los eigenvalores actuan como ponderadores para determinar la extensión de estos eigenvectores.
 
**Vectores ortogonales:** Dos vectores se consideran ortogonales cuando su producto punto:
...
Para comprender el potencial del uso de PCA para el análisis de datos primero ubiquemos el proceso que sigue esta técnica en el esquema más amplio del análisis y en seguida haremos una serie de ejemplos comenzando con una  aplicación para una matriz de 500+ , 30.
...
## Ejemplo 1 Base de datos sobre cáncer de mama. 


## PCA Intuición. Ejemplo estudio con datos sobre Cáncer de Mama.
...
Este subconjunto de variables consituyen un sístensis que representa los eigenvectores de la matriz original. 

[Datos](http://archive.ics.uci.edu/ml/datasets/breast+cancer+wisconsin+%28diagnostic%29)
```{r,echo=FALSE}
library(corrplot)
...
Sin embargo, es posible que en las 33 variables tenemos cierta correlación entre ellas. La técnica de **PCA** es útil en este caso por que podemos representar las 30 dimensiones mediante un subconjunto menor que capture la mayor variación posible contenida el la matriz. Esta reducción se basa en la trayectoria de los eigenvectores de la matriz original.  

Entre las variables tenemos una denominada diagnosis, esta indica el tipo de tumor benigno **B** o maligno **M**
...
Donde la variable que nos interesa predecir es *diagnosis*, ej. determinar si el tumor es maligno o benigno (variable dependiente) con base en las 32 variables independientes. 
Primero aplicamos el PCA para reducir las dimensiones del df. esto nos permite clasificar a los pacientes con base en los atributos y visulizar el resultado mendiante un menor número de dimensiones.   See para mayor referencia sobre el caso de estudio.https://www.kaggle.com/shravank/predicting-breast-cancer-using-pca-lda-in-r 
