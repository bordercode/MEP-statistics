...
**Note que Odds no son probabilidades.**  Para llegar a estimar la probilidades que  un evento ocurra dado un conjunto de variables explicativas el modelo de regresión logística considera una serie de supuestos adicionales, e especial es importante incorporar la función logit, que veremos enseguida. 
...
Nuevamente suponga ahora que nuestro equipo participa en un total de  8 partidos. Gana 5 pierder 3. Igualmente podemos calcular los momios de ganar como $$5/3=1.7$$ Pero la probabilidad de ganar es: $$5/8=0.625$$
...
Note que los momios a favor de ganar se incrementan a media que el número de juegos aumenta y de hecho  tienden a $\infty$. Pero esta vez se ubican en el intervalo $[1,\infty)$.  Uno si al menos gana los mismos partidos de los que pierde y tiende a $\infty$ si gana todos los juegos jugados $n/0$ (no esta definido paro tiende a **infinito**).
...
Donde el lago izquierdo es nombrado función **logit** o log Odds.
...
La estimación de los coeficientes del modelo que haremos genera precisamente el valor del **log(odds)**, no de la **probabilidad**. 
...
En palabras la función de máxima verosimilitud  realiza un proceso interativo para seleccionar la linea que maximiza el log(likelihood) calculando  el producto de  cada probabilidad de que el evento ocurra por el producto de las probabilidades estimadas de que el evento no odurra. 

Maximizar la función de verosimilitud implica determinar aquella  probabilidad $\hat{p}(x_i)$ que es lo más apegada posible al valor observado de los datos. 

En nuestro ejemplo con la base de datos sobre incumplimiento (*default*) del pago en tarjeta de crédito, buscamos los valores de $\hat{\beta_0}, \hat{\beta_1}$ que al substituirlos en la función $p(X)$, resulten en un número lo mas cercano posible a 1 para los individuos en la catergoría default y cercano a cero en caso contrario. 

##### Prueba para determinar si el log(odds ratio) es estadisticamente significativo. 
...
Importante recordar que los log odd  (log de los momios) y los log(odd) (cociente de momios), se distribuyen acorde a la **normal** con:   $N\approx(\mu=0,\sigma=1)$ y recordando los fundamentos del **Teorema de limite Central**  podemos estimar **intervalos de confianza** y  *p values* como areas de bajo de la curva de la distribución. 
...
Ahora como analogía de los modelos de RL MCO, es posible estimar una medida que nos indique en el modelos de **RL** es válido. Este ese el indicador Pseudo $R^2$ de McFadden.
$$R^2=\frac{ll(null )-ll(fit)}{ll(null)}$$
Donde:
$$ll(fit)=log(likelyhood) max$$
...
Hacemos ahora predicciones de rpobabilidades con base en los log(odds) estimados. Por ejemplo para un individuo cuyo balance=$\$1,000$
...
Una probanilidad de default de $0.57\%$. 

Ahora hacemos la preduicción para un  individuo con un saldo (*balance*) de $\$2,000$.
...
ll.fit <- logistic$deviance/-2

## McFadden's Pseudo R^2 = [ LL(Null) - LL(fit) ] / LL(Null)
(ll.null - ll.fit) / ll.null

## El p-value for the R^2
1 - pchisq(2*(ll.fit - ll.null), df=(length(logistic$coefficients)-1))
```
