...
Un punto adicional, los Odds son un ratio un cociente entre dos eventos (las veces que ocuren un resultado entre las veces que no ocurre ese resultado).
...
Note que Eliminamos el logaritmo aplicando la operación inversa el *e* para ambos lados de la equiación. En palabras la estimación de los momios es igual a la exponencial la la equaición lineal $\beta_0+\beta_1X$.
...
Que de hecho tiene una **función logit** que es lineal en x.

La estimación de los coeficientes del modelo que haremos genera precisamente el valor del **log(odds)**, no de la **probabilidad**. 

La probabilidad se obtiene a partir de substituir el coeficiente en la función de regresión logistica para valores especificos de las variables independientes **x**.

##### Estimación de coeficientes en el modelo de regresión logistica. 

Usamos el procedimiento de **Máxima verosimilitud** a diferencia del caso del modelo de regressión lineal en donde se uso **MCO**

Este permite seleccionar los coeficientes $\beta_0 , \beta_1$ que maximizan la **función de verosimilitud**.

La estimación busca que determinar aquella  probabilidad $\hat{p}(x_i)$ que sea lo más apegada posible al valor observado. 
...
##### Determinación si el log(odds ratio) es estadisticametne significativo. 
...
Notese que la hipótesis nula $H_0: \hat{\beta_1}=0$ lo que implica que

$$p(X)=\frac{e^{\beta_0}}{1+e^{\beta_0}}$$
Y tal como hacemos en cualquier prueba de hipótesis, rechazams la **hipótesis nula** si el $p-value<\alpha$ threshld de contraste, usualmente 0.05.
...
#### Ejecicio 1 Modelo de regressión logística.
...
summary(logistic)
```
Un incremento de una unidad en la variable Balance se relaciona con  un incremento de .00549 en los log(odds) de la variable *default* y esta relación es estadisticamente significativa al 0.05. 

Hacemos ahora prediciones con base en los log(odds) estimados. Por ejemplo para un individuo cuyo balance=$\$1,000$
...
Ahora hacemos lapreduicción para un  individuo con $\$2,000$.
...
Un individuo con un balance de 2000 en su tarjeta tiene una probabilidad de $58.6\%$
...
Considere el escenario de la variable dicotomica student. que toma los valores 1 si el individuo es es estudiante y 0 en caso contrario. Estimaremos el modelo igual para la variable dependiente Default y ahora con la variable independiente dicotomica  student. 

```{r}
l2<-glm(default~student, data=d, family=binomial)
summary(l2)
```
El resultado indica que de hecho que ser estudiante incrementa el  log(odds) de incumplimiento en .40.
