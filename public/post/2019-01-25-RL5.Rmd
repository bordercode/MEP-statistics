...
bwght<-mutate(bwght,gramos=bwghtlbs*0.453592*1000, finc2019=faminc*1.5838)

summary(bwght$faminc)

lm(gramos~cigs+finc2019,data=bwght)%>%
  summary()
...
Respecto a los terminos cuadráticos ´(**efectos no lineales**) estos indican laS tasas de cambio en la pendiente de  cada variable (ej. tenure  y exper), por lo que en este caso ambas variables implican una reDucción en la del efecto atribuido a la variable original. 

Así, un año adicional en el empleo genera una reducción en la pendiente de 0.05%.(efecto de $tenure^2$).   Note que en este caso el efecto constante de la variable *tenure* es un incremento de $3.1\%$, este es el cambio con un año adicional con ese empleo, pero esta ganancia decrece con cada año extra.  
...
Adicionalmente las variables $llotsize$ y $lsqrft$ muestran los cAmbios porcentuales de la variable dependiente *log(precio)* ante el cambio de $1\%$.  POr lo que estos coeficientes $\hat \beta$ de hecho permiten conocer la  **elasticidad** un concpeto ampliamente usaso en estudios osicales para indicar la flexibilidad de una variable ante variaciones en posibles determinantes.
...
Producto Interno Bruto per capita 2005-2009 (miles de USD constantes a 2005) (*gdp*), años promedio de escolaridad escolaridad (*school*), tasa de fertilidad en pobalción adolescente  (nacimientos por cada 1000, *adfert*: Adolescent fertility: births/1000 births 2005/2009). Mortalidad infantil (*chldmort*:  tasa de mortalidad infantil (antes de 5 años por cada 1000 nacimientos). 
...
Ahora podemos medir la interacción entre edad y el diagnóstico de hipertensión, un hipótesis en este sentido es que podrían presentarse diferencias en los niveles de glucosa promedio a medida que avanza la edad del individuo dependiendo si este tiene diagnóstico de hipertensión o no. 
